...
# XCode build 出来的APP文件有所优化，比 Flutter build ios 的 red_book.app 要小
...
# Xcode 的缓存目录
# 可使用命令：xcodebuild -project red_book.xcodeproj -showBuildSettings -destination "generic/platform=iOS" 查看 -json 以json 形式查看其中的 BUILD_DIR
# Xcode 中使用 Product -> Show Build Folder in Finder

# 上述命令中的 build_dir 是打包目录
# Xcode_runner_path=~/Library/Developer/Xcode/DerivedData/red_book-ecduyvhxvpunmbfrflanxlsnblwf/Build/Products/Release-iphoneos/red_book.app
Xcode_runner_path=${project_path}/build/ios/iphoneos/red_book.app

#-------------------可选：自己的plist配置路径------------------#
# export_plist_path=${project_path}/shell/scriptTest.plist
export_plist_path=${project_path}/ios/red_book/info.plist

#-------------------可选：修改为自己的APP名称------------------#
app_name="小红书"

#----------------可选：将 red_book 替换成自己的工程名(.xcodeproj 文件)---------------#
project_name=red_book

#----------------可选：将Runner替换成自己的 sheme 名--------------#
scheme_name=red_book
...
if [ $number == 0 ];then
  ...
  #info.plist路径
  info_plist="Payload/red_book.app/info.plist"
